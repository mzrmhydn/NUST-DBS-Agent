...
1. **Identity drift.** An applicant who becomes a student is often represented by two unrelated rows in two unrelated systems. When the student later applies for a scholarship that requires their entry-test score, the office has to manually reconcile names, CNICs, and dates of birth.
2. **Curriculum duplication.** A shared foundational course like *Programming Fundamentals* (CS118) is delivered to BSCS, BESE, and BEE students together in the same classroom. Systems that model courses as belonging to a program instead of to a teaching school (a *school* owns the instructor and the classroom) end up storing CS118 three times — and when its credit hours change, three rows need to be updated in lockstep.
3. **State collapse.** Common systems overload the `Grade` column with sentinel values like `'IP'` (in progress) or `'W'` (withdrawn) to avoid adding a second column. This breaks every average and every sort, and forces the application layer to re-encode the business state on every read.
4. **Implicit invariants.** Rules like *"an enrolled application cannot also be a rejected one"* or *"a classroom cannot be overbooked"* are usually enforced in application code that a different team maintains. The database itself is indifferent — which is how conflicting states accumulate silently when two admissions officers act simultaneously.

### 1.3 Our solution

This project designs and implements a **single normalized MySQL database** — `nust_university` — that treats the admissions pipeline and the academic pipeline as one domain. Student identity is anchored to a unique `applicant_id` — every student row carries a direct FK to both their applicant record and their admitted program, backed by a trigger (`auto_update_application_status`) that synchronizes application and offer status on admission. Course ownership is moved from program to school, and a true many-to-many `program_course` junction carries an `is_core` BOOLEAN and a recommended `semester`. In-progress enrollments are distinguished from graded ones by a nullable `grade` column rather than a sentinel value. Every invariant the business cares about is expressed as a declarative constraint, not inferred from application code.

The schema is intentionally **narrow in scope and deep in correctness** — 16 entities, four of which are junction tables — covering the complete journey from a candidate's first NET attempt through their final term's letter grade. A FastAPI backend (`api.py`) and a React frontend expose the database through a natural-language interface backed by a local Ollama LLM, so non-technical staff can query admissions status, course prerequisites, section staffing, and enrollment grades without writing SQL.
...
| FR-23 | The system shall support reporting of per-school conversion rate from Applicant to Student, per-Program average NET score of selected applicants, and per-school section staffing — without any denormalized cache columns. |
| FR-24 | The system shall expose at least one **stored procedure** for automating a multi-step admission workflow, and at least one **function** (`IsEligibleForEngineering`) that returns whether an applicant has cleared the Engineering NET threshold. |
| FR-25 | The system shall support atomic **transactions** over multi-table admission workflows (e.g., creating a `student` row and having the trigger automatically update `application` and `offer` status) with `ROLLBACK` on any failure. |
...
| FR-27 | The system shall embed user questions using the same embedding model used to build the index, so that distance metrics are comparable. |
| FR-28 | The system shall retrieve the top-4 Q&A example pairs most relevant to the user's question before generating SQL. |
| FR-29 | The system shall pass retrieved context plus user question to a locally-hosted LLM (`llama3.1` via Ollama) and must not transmit any query content to external services. |
...
| FR-33 | The system shall allow the knowledge base to be updated by editing `examples/examples.json` and restarting `api.py`; no separate build step is required. |
...
| NFR-7 | The FAISS index shall be **rebuilt from `examples/examples.json`** on each API server startup; the build completes in under 5 seconds and requires no GPU. |
